...
namespace Rave {
    ...
    Arena Arena::create() {
        ...
        if (a.start == MAP_FAILED) a.start = nullptr;
#endif
    ...
    void Arena::align(size_t alignment) {
...
#else
            void* result = mmap((char*)start + committed, commit,
    ...
    void Arena::pop_to(void* pos) {
        if (pos >= start && pos <= current) {
            current = pos;
        }
    }

    void Arena::pop_to(size_t size) {
    ...
    ArenaBlock* ArenaBlock::create(Arena* a, size_t size, size_t alignment) {
        ArenaBlock* hdr = a->push<ArenaBlock>();
        if (!hdr) return nullptr;
        hdr->arena    = a;
        hdr->start    = a->push(size, alignment);
        if (!hdr->start) return nullptr;
    ...
    void* ArenaBlock::capacity() {
        return end;
    }

    void ArenaBlock::align(size_t alignment) {
        size_t mask = alignment - 1;
        current = (void*)(((uintptr_t)current + mask) & ~mask);
    }

    void* ArenaBlock::push(size_t size, size_t alignment) {
    ...
    bool ArenaBlock::can_push(size_t size, size_t alignment) {
        RAVE_ASSERT(start >= arena->start && end <= arena->current);
        size_t mask = alignment - 1;
    ...
    void ArenaBlock::pop_to(void* pos) {
        if (pos >= start && pos <= current) {
            current = pos;
        }
    }

    void ArenaBlock::pop_to(size_t size) {
        void* target = (void*)((uintptr_t)current - size);
        if (target >= start) {
            current = target;
        }
